...
class addtocours4 extends StatefulWidget {
  const addtocours4({Key? key}) : super(key: key);
  @override
...
class _addtocours4State extends State<addtocours4> {
 ...
 void pickFile() async {
   ...
   if(pickedFile != null){
    ...
    await   _firebaseFirestore.collection("pdfs").doc(fileName).set({
       "name" : fileName,
       "url": downloadLink
     });
    ...
    body: GridView.builder(
      itemCount: pdfData.length,
        ...
        return Padding(
          ...
          child: InkWell(
            onLongPress: ()async{
              await _firebaseFirestore.collection('pdfs').doc(pdfData[index]["name"]).delete();
            },
            onTap: (){
